...
def LRU_Free_Register(value):
    ...
    if((value in common_variable_lines_mapping and value not in source_variable_lines_mapping) or 
       (len(common_variable_lines_mapping[value]) >= len(source_variable_lines_mapping[value]))):
...
def assembly_gen(instructions):
    ...
    for i in range(len(instructions)):
        ...
        if(instructions[i][0] == "Label"):
            reg_dealloc()
            # Check if instruction has enough elements
            ...
            if len(instructions[i]) >= 4:
                label = instructions[i][3]
            elif len(instructions[i]) >= 2:
                label = instructions[i][1]  # Use second element as label
            else:
                continue  # Skip malformed instruction
            
            print("BR " + label)
            output_file.write("BR " + label + "\n")
